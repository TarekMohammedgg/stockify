...
**Status note (2026-05-26):** Order submission and insights persistence are implemented inline in `app/api/chat/route.ts` (writes to `orders` + `order_items` + upserts `chatbot_insights`). Greeting branches on insights presence in `components/public/chatbot-widget.tsx`. Remaining work is hardening + verification, ordered by dependency below.
...
- [x] Submit structured order (direct insert in `/api/chat` — functionally equivalent to `/api/orders`)
- [x] Update `chatbot_insights` (favourite_items, default_address, last_seen)
- [x] New-vs-returning branching: greeting in widget switches on insights; system prompt receives stored phone/address/favourites
